...
local sv_gravity = GetConVar("sv_gravity"):GetInt() * -1
local accel = Vector(0, 0, sv_gravity)
...
function ACF_CreateBullet(BulletData)
	...
	BulletData = table_Copy(BulletData) -- this is required to avoid overwritting the origin table

	-- Those are BulletData settings that are global and shouldn't change round to round
	BulletData.Gravity = sv_gravity
	BulletData.Accel = accel
	...
	if IsValid(Parent) then
		...
		if IsValid(physObj) then BulletData.TraceBackComp = math_max(physObj:GetVelocity():Dot(BulletData.Flight:GetNormalized()), 0) end
	end

	if BulletData.Filter then
		table_Add(BulletData.Filter, {BulletData.Gun})
	else
		BulletData.Filter = {BulletData.Gun}
	end

	BulletData.Index = ACF.CurBulletIndex
	ACF.Bullet[ACF.CurBulletIndex] = table_Copy(BulletData) -- Place the bullet at the current index pos
...
do
	...
	function ACF_CalcBulletFlight(Index, Bullet, BackTraceOverride)
		...
		local Water = util_TraceLine(WaterTr)

		Bullet.UnderWater = false

		if Water.HitWorld and Water.StartSolid then Bullet.UnderWater = true end

		-- actual motion of the bullet
		local Drag = NormFlight * (Bullet.DragCoef * FlightLength ^ 2) / ACF.DragDiv
		if Bullet.UnderWater then Drag = Drag * 800 end
		...
		Bullet.StartTrace = Bullet.Pos - Flightnorm * math_min(PhysVel, Bullet.FlightTime * FlightLength - Bullet.TraceBackComp * Bullet.DeltaTime)
		Bullet.EndTrace = Bullet.NextPos + Flightnorm * PhysVel

		debugoverlay.Cross(Bullet.Pos, 5, DebugTime, color_white) -- true start
		debugoverlay.Line(Bullet.Pos, Bullet.NextPos, DebugTime, color_green, true) -- the predicted trayectory.
		debugoverlay.Line(Bullet.StartTrace, Bullet.EndTrace, DebugTime, color_yellow) -- the real trace detection.
...
do
	...
	local function ACF_PerformTrace(Bullet)
		...
		while RetryTrace and visCount < MaxvisclipPerBullet do
			...
			if FlightRes.HitNonWorld and ACF_CheckClips(FlightRes.Entity, FlightRes.HitPos) then -- our shells hit the visclip as traceline, no more double bounds.
				...
				table_insert(Bullet.Filter, FlightRes.Entity)
			...
			if FlightRes.HitNonWorld and (HitEnt:IsPlayer() or HitEnt:IsNPC()) then
				...
				if HitEnt ~= PlayerHitCheck then
					table_insert(Bullet.Filter, HitEnt)
					table_insert(Bullet.FilterKeysToRemove, #Bullet.Filter)
	...
	do
		...
		local Hit_Resolutions = {
			...
			Ricochet = function(Index, Bullet, FlightRes, type)
				...
				if isProp then Bullet.ImpactCount = (Bullet.ImpactCount or 0) + 1 end
	...
	function ACF_DoBulletsFlight(Index, Bullet)
		...
		if Bullet.FuseLength and Bullet.FuseLength > 0 and Bullet.FlightTime > Bullet.FuseLength then
			...
			if FlightRes.Hit and FlightRes.Fraction < ratio or Bullet.HasPenned then ScaledPos = FlightRes.HitPos end

			if not util_IsInWorld(ScaledPos) then
				...
				debugoverlay.Sphere(ScaledPos, 10, DebugTime, clr)
		...
		if ACF_DoOnBulletFlight then ACF_DoOnBulletFlight(Index, Bullet) end
		...
		elseif FlightRes.HitNonWorld then
			...
			local engine_T = engine_TickInterval()
			local DTImpact = ((FlightRes.HitPos - Bullet.Pos):Length() / (Bullet.Flight * ACF.VelScale * engine_T):Length()) * engine_T -- i would rather use tickinterval over deltatime
			...
			if ACF.TraceFilter[FlightRes.Entity:GetClass()] and Retry == "Penetrated" then Retry = false end
...
function ACF_BulletClient(Index, Bullet, Type, Hit, HitPos)
	...
	elseif Type == "Init" then
		...
		if not IsValid(Bullet.Gun) or Bullet.Gun:GetClass() == "ace_missile" then IsMissile = 1 end
