...
local legal = ACF.Legal
ACF.Legal.NextCheck = function(_, Legal)
	return ACF.CurTime + (Legal and math_random(legal.Min, legal.Max) or legal.Lockout)
end
...
do
	...
	local AllowedMaterials = {RHA = true, CHA = true, Alum = true}

	local ValidCollisionGroups = {[COLLISION_GROUP_NONE] = true, [COLLISION_GROUP_WORLD] = true, [COLLISION_GROUP_VEHICLE] = true}
	...
	function ACF_CheckLegal(Ent, Model, MinMass, MinInertia, _, CanVisclip)
		...
		if ACF.Legal.Ignore.Solid <= 0 and not Ent:IsSolid() then table.insert(problems, "Not solid") end

		-- check if the model matches
		if Model ~= nil and ACF.Legal.Ignore.Model <= 0 and Ent:GetModel() ~= Model then table.insert(problems, "Wrong model") end
		...
		if ACF.Legal.Ignore.Mass <= 0 then
			...
			if MinMass ~= nil and CMass < MinMass then table.insert(problems, "Under min mass") end
		...
		if ACF.Legal.Ignore.Material <= 0 then
			...
			if not AllowedMaterials[material] then table.insert(problems, "Material not legal") end
		end
		...
		if ACF.Legal.Ignore.Inertia <= 0 and MinInertia ~= nil then
			...
			if (inertia.x < MinInertia.x) or (inertia.y < MinInertia.y) or (inertia.z < MinInertia.z) then table.insert(problems, "Under min inertia") end
		end

		-- check makesphere
		if ACF.Legal.Ignore.makesphere <= 0 and physobj:GetVolume() == nil then table.insert(problems, "Has makesphere") end

		-- check for clips
		if ACF.Legal.Ignore.visclip <= 0 and not CanVisclip and (Ent.ClipData ~= nil) and (#Ent.ClipData > 0) then table.insert(problems, "Has visclip") end

		-- check for bad collision groups
		if ACF.Legal.Ignore.Solid <= 0 and not ValidCollisionGroups[Ent:GetCollisionGroup()] then table.insert(problems, "Bad collision group") end
